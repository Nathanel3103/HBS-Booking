...
async function connectToDatabase() {
  ...
  try {
    ...
    cachedDb = cachedClient.db('healthcare');
    return cachedDb;
  } catch (error) {
    cachedClient = null;
    cachedDb = null;
...
async function handleDateSelection(message, sender, db) {
  try {
    ...
    if (!message || typeof message !== 'string' || !validateDate(message)) {
      return WHATSAPP_CONFIG.ERRORS.INVALID_DATE;
    }

    // Parse date components
    const dateParts = message.match(/^(\d{1,2})\/(\d{1,2})$/);
...
function isSessionExpired(userState) {
  ...
  console.debug('Session expiry check:', {
    now,
    lastUpdated,
    difference: now - lastUpdated,
    timeoutMs,
    isExpired: now - lastUpdated > timeoutMs
  });
...
async function getAvailableDoctorsResponse(db, date) {
  try {
    ...
    const doctors = await db.collection(WHATSAPP_CONFIG.COLLECTIONS.DOCTORS)
      .find({ 
        availableDays: { $in: [selectedDate.getDay()] } 
      })
      .toArray();
    ...
    if (doctors.length === 0) {
      ...
      try {
        ...
        for (let i = 1; i <= 7; i++) {
          ...
          const availableDoctors = await db.collection(WHATSAPP_CONFIG.COLLECTIONS.DOCTORS)
            .find({ 
              availableDays: { $in: [nextDate.getDay()] } 
            })
            .toArray();
        ...
        if (availableDates.length > 0) {
          let message = `${WHATSAPP_CONFIG.ERRORS.NO_DOCTORS_AVAILABLE}\n\n`;
          message += `Here are dates with available doctors:\n`;
          message += availableDates.map(d => `- ${d.formattedDate}`).join('\n');
          message += `\n\nPlease reply with your preferred date (e.g. ${formatDate(new Date())})`;
          return message;
        }
      } catch (error) {
        console.error('Error checking alternative dates:', error);
      }
      return WHATSAPP_CONFIG.ERRORS.NO_AVAILABILITY_EXTENDED;
    }
...
function generateWelcomeMessage() {
  return `👋 Welcome to ${WHATSAPP_CONFIG.HOSPITAL_NAME}!
