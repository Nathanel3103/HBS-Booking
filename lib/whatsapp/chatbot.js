...
async function handleDoctorSelection(message, sender, db) {
  try {
    ...
    // Update user state with selected doctor
    await db.collection(WHATSAPP_CONFIG.COLLECTIONS.CHAT_STATES).updateOne(
      { sender },
      { 
        $set: { 
    ...
    // Format available time slots for response
    const timeSlotsMessage = selectedDoctor.availableSlots
      .map((slot, index) => `${index + 1}. ${slot}`)
      .join('\n');

    return `🕒 Please select a time slot for Dr. ${selectedDoctor.name}:\n${timeSlotsMessage}`;
  } catch (error) {
