...
async function handleDoctorSelection(message, sender, db) {
  try {
    ...
    if (isNaN(selectedIndex)) return WHATSAPP_CONFIG.ERRORS.INVALID_DOCTOR;

    const userState = await db.collection(WHATSAPP_CONFIG.COLLECTIONS.CHAT_STATES).findOne({ sender });
    if (!userState) return WHATSAPP_CONFIG.ERRORS.SESSION_EXPIRED;

    const doctors = await db.collection('doctors').find({
      'workingHours.day': userState.date
    }).toArray();
    ...
    const timeSlotsData = await getAvailableTimeSlotsResponse(
      db, 
      doctors[selectedIndex]._id, 
      userState.date
    );

    if (typeof timeSlotsData === 'string') return timeSlotsData;
    ...
    await db.collection(WHATSAPP_CONFIG.COLLECTIONS.CHAT_STATES).updateOne(
      ...
      { 
        $set: { 
          ...
          doctorId: timeSlotsData.doctorId,
          doctorName: timeSlotsData.doctorName,
          doctorSpecialization: timeSlotsData.specialization,
          availableSlots: timeSlotsData.slots
        }
      }
    );

    const timeSlotsMessage = timeSlotsData.slots
      .map((slot, index) => `${index + 1}. ${formatTimeSlot(slot)}`)
      .join('\n');

    return `🕒 Available slots for Dr. ${timeSlotsData.doctorName} (${timeSlotsData.specialization}):\n${timeSlotsMessage}\n\nPlease reply with the number of your preferred time slot`;
...
async function getAvailableTimeSlotsResponse(db, doctorId, date) {
  try {
    const doctor = await db.collection('doctors').aggregate([
      {
        $match: { 
          _id: doctorId,
          'workingHours.day': date 
        }
      },
      {
        $lookup: {
    ...
    if (!doctor) return WHATSAPP_CONFIG.ERRORS.DOCTOR_NOT_AVAILABLE;
    if (!doctor.availableSlots?.length) return WHATSAPP_CONFIG.ERRORS.NO_SLOTS_AVAILABLE;

    const bookedSlots = doctor.appointmentsBooked
    ...
    return availableSlots.length ? {
      ...
      doctorId: doctor._id
    } : WHATSAPP_CONFIG.ERRORS.NO_SLOTS_AVAILABLE;
  } catch (error) {
...
function formatTimeSlot(slot) {
  return `${slot.substring(0, 2)}:${slot.substring(2, 4)} - ${slot.substring(5, 7)}:${slot.substring(7, 9)}`;
}
