...
export function validateUserState(userState) {
  if (!userState) return false;
  if (!userState.step) return false;
  if (!userState.lastUpdated) return false;
  
  const validSteps = ['service_selection', 'date_selection', 'time_selection', 'doctor_selection', 'confirmation'];
  return validSteps.includes(userState.step);
}
